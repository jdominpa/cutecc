...
// numbers, strings and characters include their own source text.
const char *token_to_str(Token t)
{
    ...
    case TK_CHAR:
        snprintf(buf, sizeof(buf),
                 "%s `%.*s`", token_kind_to_str[t.kind], (int) t.len, t.start);
        return buf;
    default:
        snprintf(buf, sizeof(buf), "`%s`", token_kind_to_str[t.kind]);
        return buf;
    }
}
